...
def test_lazy_imports():
    ...
    assert "EmptySpectrumWarning" in dir(imzy)
    assert "H5CentroidsStore" in dir(imzy)
    assert "get_reader" in dir(imzy)

    from imzy import BaseReader, EmptySpectrumWarning, H5CentroidsStore, get_reader

    assert BaseReader is not None, "BaseReader should be imported"
    assert EmptySpectrumWarning is not None, "EmptySpectrumWarning should be imported"
